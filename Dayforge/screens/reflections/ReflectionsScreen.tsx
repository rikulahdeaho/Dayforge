      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {state.reflectionHistory.length ? (
          state.reflectionHistory.map((entry, index) => (
            <Pressable key={entry.id} onPress={() => router.push({ pathname: '/reflection/[id]' as never, params: { id: entry.id } } as never)}>
              <SurfaceCard palette={palette} style={[styles.entryCard, index === 0 ? { borderColor: palette.accent, borderWidth: 1.6 } : null]}>
                <View style={styles.entryRow}>
                  <Text style={styles.emoji}>{moodEmoji(entry.mood)}</Text>
                  <View style={styles.entryCopy}>
                    <Text style={[styles.entryDate, { color: palette.text }]}>{entry.fullDate}{index === 0 ? '  •  Latest' : ''}</Text>
                    <Text numberOfLines={2} style={[styles.entryPreview, { color: palette.mutedText }]}>{entry.preview}</Text>
                    <Text numberOfLines={1} style={[styles.entryMoodSummary, { color: palette.accentSoft }]}>{moodSummary(entry.mood)}</Text>
                  </View>
                  <SymbolView name={resolveSymbolName({ ios: 'chevron.right', android: 'chevron_right', web: 'chevron_right' })} size={18} tintColor={palette.mutedText} />
                </View>
              </SurfaceCard>
            </Pressable>
          ))
...
const styles = StyleSheet.create({
  ...
  headerRow: { marginBottom: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  title: { ...Type.screenTitle },
  closeButton: { width: 48, height: 48, borderRadius: 24, borderWidth: 1, alignItems: 'center', justifyContent: 'center' },
  entryCard: { marginBottom: 10, borderRadius: 24 },
  entryRow: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  emoji: { fontSize: 28 },
  entryCopy: { flex: 1 },
  entryDate: { ...Type.cardTitle, marginBottom: 4 },
  entryPreview: { ...Type.bodySmall },
  entryMoodSummary: { marginTop: 3, ...Type.metaStrong },
