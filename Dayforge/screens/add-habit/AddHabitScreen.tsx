...
const habitIconOptions = [
  { id: 'figure.mind.and.body', label: 'Mind', icon: { ios: 'figure.mind.and.body', android: 'self_improvement', web: 'self_improvement' } },
  { id: 'book.fill', label: 'Read', icon: { ios: 'book.fill', android: 'menu_book', web: 'menu_book' } },
  { id: 'drop.fill', label: 'Water', icon: { ios: 'drop.fill', android: 'water_drop', web: 'water_drop' } },
  { id: 'dumbbell.fill', label: 'Workout', icon: { ios: 'dumbbell.fill', android: 'fitness_center', web: 'fitness_center' } },
  { id: 'moon.stars.fill', label: 'Sleep', icon: { ios: 'moon.stars.fill', android: 'bedtime', web: 'bedtime' } },
  { id: 'heart.fill', label: 'Health', icon: { ios: 'heart.fill', android: 'favorite', web: 'favorite' } },
];

export default function AddHabitScreen() {
  const params = useLocalSearchParams<{ mode?: string; habitId?: string; title?: string; subtitle?: string; iconId?: string }>();
  ...
  useEffect(() => {
    if (!isEditing) return;
    if (typeof params.title === 'string') setHabitTitle(params.title);
    if (typeof params.subtitle === 'string') setHabitSubtitle(params.subtitle);
  ...
  const saveHabit = () => {
    ...
    if (isEditing && typeof params.habitId === 'string') {
      updateHabit({ habitId: params.habitId, title: habitTitle, subtitle: habitSubtitle, icon: selectedIcon });
    } else {
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.safe}>
        <ScrollView ref={scrollRef} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          <View style={styles.headerRow}>
            ...
            <Text style={[styles.subtitle, { color: palette.mutedText }]}>{isEditing ? 'Update name, subtitle and icon' : 'Set a name, subtitle and icon'}</Text>
          </View>
          ...
          <SurfaceCard palette={palette} style={styles.formCard}>
            ...
            <View style={styles.actionRow}>
              ...
              <Pressable onPress={saveHabit} disabled={!habitTitle.trim()} style={[styles.primaryBtn, { backgroundColor: palette.accent, opacity: habitTitle.trim() ? 1 : 0.5 }]}>
                <Text style={[styles.primaryBtnText, { color: palette.onAccent }]}>{isEditing ? 'Save Changes' : 'Save Habit'}</Text>
              </Pressable>
            </View>
          </SurfaceCard>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  label: { ...Type.label, marginBottom: 8, marginTop: 2 },
  input: { borderWidth: 1, borderRadius: 14, paddingHorizontal: 14, paddingVertical: 14, ...Type.body, marginBottom: 10 },
  iconGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 2 },
  iconOption: { minWidth: 98, borderWidth: 1, borderRadius: 14, paddingHorizontal: 10, paddingVertical: 10, flexDirection: 'row', alignItems: 'center', gap: 8 },
  iconLabel: { ...Type.metaStrong },
